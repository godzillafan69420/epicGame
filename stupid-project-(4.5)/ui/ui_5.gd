...
@onready var ivan = preload("res://images/ivanBubble.png")
...
func _process(_delta: float) -> void:
	...
	if get_parent().gamePhase == 1:
		...
		if GlobalVariables.character ==1:
			$barryDialog.texture = ivan
			panel.visible = true
			barrySprite.visible = true
			if textToShow == 0:
				textBox.text = "Huh what are you doing stop killing the goons
				How am i suppose to kidnap children without goons"
			if textToShow == 1:
				textBox.text = "Do it solo"
				barrySprite.visible = false
				protagonistDialog.visible = true
			if textToShow == 2:
				textBox.text = "Bruh, noooo"
				barrySprite.visible = true
				protagonistDialog.visible = false
			if textToShow == 3:
				textBox.text = "Little bro you need some exercise not gonna lie"
				barrySprite.visible = false
				protagonistDialog.visible = true
			if textToShow == 4:
				textBox.text = "You want to pick a fight with me.
				Huh, trying to mess with the chess gang huh.
				Think you tough huh"
				barrySprite.visible = true
				protagonistDialog.visible = false
			if textToShow == 5:
				textBox.text = "erm chill"
				barrySprite.visible = false
				protagonistDialog.visible = true
			
			if textToShow == 6:
				textBox.text = "Nah you and me right here right now"
				barrySprite.visible = true
				protagonistDialog.visible = false
			
			if textToShow == 7:
				textBox.text = "This is getting old"
				barrySprite.visible = false
				protagonistDialog.visible = true
			if textToShow == 8:
