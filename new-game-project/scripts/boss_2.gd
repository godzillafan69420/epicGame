...
const shooter_timer_wait_timer = 0.3
...
func _process(delta):
	...
	if bossPhase == 3 and count2 != 1:
		$"3Phaseattack".start()
		count2 = 1
	if Hitpoint < 80000 and Hitpoint > 40000:
		bossPhase = 2
	elif Hitpoint < 40000:
		bossPhase = 3
...
func _on_shoot_towards_the_player_timeout() -> void:
	if bossPhase ==3:
		for i in range(25):
			var bullets = bulletPrefab.instantiate()
			bullets.position = Vector2(-100* i +randf_range(-100,100), -540)
...
func _on_bullet_shootin_everywhere_timeout() -> void:
	for i in range(10):
		if thePlayer != null and bossPhase == 1:
